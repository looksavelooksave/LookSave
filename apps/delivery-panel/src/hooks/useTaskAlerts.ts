...
const LS_KEY = 'looksave.delivery.alerts';
...
export function useTaskAlerts(pendingTasks: Array<{ id: string }>): {
  enabled: boolean;
  permission: Perm;
  toggle: () => void;
} {
  ...
  useEffect(() => {
    ...
    if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
      const body =
        fresh.length === 1 ? `Yangi buyurtma #${fresh[0]!.slice(0, 8)}` : `${fresh.length} ta yangi buyurtma`;
      const note = new Notification('Yangi buyurtma — Dostavka', { body, tag: 'looksave-queue' });
      note.onclick = (): void => {
        window.focus();
        note.close();
      };
