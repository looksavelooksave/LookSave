...
export function LoginPage(): JSX.Element {
  ...
  async function submit(event: FormEvent): Promise<void> {
    ...
    try {
      ...
      if (user.role !== 'admin') {
        setError('Bu panel dostavka firmasi uchun.');
        return;
      }
  ...
  return (
    <main className="flex min-h-full items-center justify-center bg-background px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <h1 className="mt-6 text-center text-2xl font-bold text-foreground">Yetkazib berish</h1>
        <p className="mt-2 text-center text-sm text-dim">Dostavka paneli</p>
